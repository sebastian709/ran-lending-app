...
class PaymentController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['to_pay'] = DB::table('loan_tenure as lt')
            ...
            ->where(function ($query) {
                $query->where('lt.payment_status_id', '<=', 1)->orWhere('lti.payment_status_id', '<=', 1);
            })
    ...
    public function submit(Request $request){
        ...
        $paymentDue  = json_decode($request->paymentDue, true) ?? [];
        $paymentPar  = json_decode($request->paymentPar, true) ?? [];
        $paymentData = json_decode($request->paymentData, true) ?? [];

        $type = 4;

        if ($request->fullpayment == 1) {
            $type = 1;
        } elseif (!empty($paymentPar)) {
            $type = 2;
        ...
        if ($request->hasFile('attachment')) {
            ...
            $directory = public_path("storage/uploads/payments");
            ...
            $attachmentPath = "uploads/payments/{$filename}";
        }
        ...
        if ($type == 1) {
            ...
            foreach ($paymentData as $record) {
                ...
                loan_tenure_interest::where('tenure_id', $record['id'])
                    ...
                    ->update([
                        'payment_id' => $Loan_payment,
                        'payment_status_id' => 4, // Different for advanced
                    ]);
            }
    ...
    {
        $paymentHistory = DB::select("
            SELECT 
                lt.id AS tenure_id,
                lp.id AS payment_id,
                COALESCE(ROUND(lt.principal + lti.interest + COALESCE(ltp.penalty, 0), 2), 0) AS amount_due,
                lps.type AS status,
                DATE_FORMAT(lp.created_at, '%M %e, %Y') AS date_of_payments,
                reference_code AS reference_number,
                COALESCE(DATE_FORMAT(lt.date, '%M %Y'), 'n/a') AS month_coverage,
                lpt.type AS payment_category,
                COALESCE(ROUND(lt.principal + lti.interest + COALESCE(ltp.penalty, 0), 2),0) AS monthly_due,
                COALESCE((CASE
                    WHEN COALESCE(lt.principal, 0) = 0 || COALESCE(lti.interest, 0) = 0 THEN lt.principal + lti.interest
                    ELSE 0
                END),0) partial,
                COALESCE(lti.interest, 0) AS interest,
                COALESCE(ltp.penalty, 0) AS penalty,
         ...
         * Add breakdown structure + comments = []
         */
        $paymentHistory = array_map(function ($p) {

            return [
                "tenure_id" => $p->tenure_id,
                "payment_id" => $p->payment_id,
                "amount_due" => (float) $p->amount_due,
                "status" => $p->status,
                "date_of_payment" => $p->date_of_payments,
                "reference_number" => $p->reference_number,
                "month_coverage" => $p->month_coverage,
                "payment_category" => $p->payment_category,

                "breakdown" => [
                    "monthly_due" => (float) $p->monthly_due,
                    "partial" => (float) $p->partial,
        ...
        return view('borrower.pages.payments.payment-history', compact('paymentHistory'));
    }
     ...
     public function getAttachment(Request $request)
    {
        ...
        return response()->json([
            'attachment' => $payment ? $payment->attachment : null
        ]);
    }
